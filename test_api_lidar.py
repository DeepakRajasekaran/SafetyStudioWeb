...
d = res.json()
print("Lidars length:", len(d.get('lidars', [])))
for l in d.get('lidars', []):
    print(f"Lidar {l.get('name')}:")
    print(f"  clip_wkt exists: {bool(l.get('clip_wkt'))}")
    print(f"  w_clip_wkt exists: {bool(l.get('w_clip_wkt'))}")
    if l.get('w_clip_wkt'):
        print(f"  w_clip length: {len(l.get('w_clip_wkt'))}")
